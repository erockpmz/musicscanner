#!/usr/bin/env python3
"""
Walkman Playlist Builder - direct to mounted Walkman

Local web app to browse a music folder, search/select tracks, and save playlists
as .m3u8 files directly to a mounted Sony Walkman or similar player.

Requirements
    pip install flask mutagen

Example 1: auto-detect Walkman on macOS
    python3 walkman_playlist_builder_direct.py \
      --music-root '/Users/elorson/Library/CloudStorage/Dropbox/Music/Main Catalog'

Example 2: explicitly point at the Walkman mount
    python3 walkman_playlist_builder_direct.py \
      --music-root '/Users/elorson/Library/CloudStorage/Dropbox/Music/Main Catalog' \
...
class WalkmanTarget:
    def __init__(self, walkman_root: Path, playlist_dir: Path) -> None:
...
def resolve_walkman_target(walkman_root: Optional[Path], playlist_subdir: Optional[str]) -> WalkmanTarget:
    ...
    return WalkmanTarget(root, playlist_dir)
...
<head>
  ...
  <style>
    ...
    .info { margin-bottom: 12px; color: #bbb; font-size: 13px; }
  </style>
</head>
<body>
  <div class="wrap">
...
async function loadDeviceInfo() {
  ...
  document.getElementById('deviceInfo').textContent = `Walkman: ${data.walkman_root} | Playlist folder: ${data.playlist_dir}`;
}
...
def create_app(library: TrackLibrary, target: WalkmanTarget) -> Flask:
    ...
    def api_search():
        ...
        return jsonify({"total": len(library.all_tracks()), "results": results})
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--music-root", required=True, help="Root folder containing source music files")
...
def main() -> None:
    ...
    if not music_root.exists() or not music_root.is_dir():
        raise SystemExit(f"Music root does not exist or is not a directory: {music_root}")
    ...
    library = TrackLibrary(music_root)
    count = library.scan()
    print(f"Scanned {count} tracks from: {music_root}")
    print(f"Walkman root: {target.walkman_root}")
    print(f"Playlist output directory on Walkman: {target.playlist_dir}")
