#!/usr/bin/env python3
"""
Rename/reorganize Walkman music files from embedded metadata. v0.1.2
...
def scan_tracks(walkman_root: Path, artist_filter: Optional[str], skip_artists: Sequence[str]) -> List[TrackInfo]:
    tracks: List[TrackInfo] = []
    current_artist_folder = None
    scanned_files = 0
    skip_set = set(skip_artists or [])

    for dirpath, _, filenames in os.walk(walkman_root):
        current_dir = Path(dirpath)
        artist_folder = current_dir.relative_to(walkman_root).parts[0] if current_dir != walkman_root and current_dir.relative_to(walkman_root).parts else None
        if artist_folder != current_artist_folder and artist_folder is not None:
            current_artist_folder = artist_folder
            if artist_folder in skip_set:
                print(f"Skipping artist folder: {artist_folder}", flush=True)
            else:
                print(f"Scanning artist folder: {artist_folder}", flush=True)
        ...
        for filename in filenames:
            ...
            if scanned_files % 100 == 0:
                print(f"  Scanned {scanned_files} tracks so far", flush=True)
...
def reorganize(
    ...
    skip_artists: Sequence[str],
    include_disc_prefix: bool,
    apply_changes: bool,
) -> Dict[str, object]:
    tracks = scan_tracks(walkman_root, artist_filter=artist_filter, skip_artists=skip_artists)
    report: Dict[str, object] = {
        "walkman_root": str(walkman_root),
        "artist_filter": artist_filter,
        "skip_artists": list(skip_artists or []),
    ...
    if apply_changes:
        ...
        for idx, item in enumerate(moves, start=1):
            ...
            if item["artist"] != current_artist:
                ...
                print(f"Working on artist: {current_artist}", flush=True)
                current_album = None
            if item["album"] != current_album:
                current_album = item["album"]
                print(f"  Working on album: {current_album}", flush=True)

            print(f"    Move {idx}/{len(moves)}: {src.name} -> {dst.name}", flush=True)
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    parser.add_argument("--skip-artist", action="append", default=[], help='Artist name to skip completely. Repeat as needed.')
