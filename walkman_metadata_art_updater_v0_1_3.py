...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    parser.add_argument("--album", help="Optional: only process one album folder name under the selected artist")
    parser.add_argument("--skip-artist", action="append", default=[], help="Artist folder name to skip. Repeat as needed.")
    ...
    if args.album:
        album_dirs = [x for x in album_dirs if x[2] == args.album]
    skipped_artists = set(args.skip_artist or [])
    if skipped_artists:
        album_dirs = [x for x in album_dirs if x[1] not in skipped_artists]
    ...
    for idx, (album_dir, artist_name, album_name) in enumerate(album_dirs, start=1):
        if artist_name != current_artist:
            current_artist = artist_name
            print(f"Working on artist: {artist_name}", flush=True)
        print(f"  Album {idx}/{total_albums}: {album_name}", flush=True)
        ...
        if res.get("skipped"):
            ...
            updated_albums += 1
            mr = res.get("matched_release", {})
            matched_title = mr.get("album_title", "")
            confidence = mr.get("confidence", 0.0)
            cover = "yes" if res.get("cover_art_found") else "no"
            print(f"    Matched: {matched_title} | confidence={confidence:.2f} | cover={cover}", flush=True)

    summary = {
        "albums_scanned": len(album_dirs),
        "albums_updated_or_ready": updated_albums,
        "albums_skipped": skipped,
        "skipped_artists_filter": list(args.skip_artist or []),
